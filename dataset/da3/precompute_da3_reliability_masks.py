...
DEFAULT_OUTPUT_DIR = (
    ...
    "cil-visionavengers-depth/teacher_masks/da3_giant_p95_square128_seed42"
)
...
def load_target_depth(depth_path, grid, mask_size):
    depth = np.load(depth_path).astype(np.float32)
    if grid == "raw_depth":
        return depth
    if grid == "square":
        return resize_array_to_shape(depth, (mask_size, mask_size), mode="nearest")
...
def parse_args():
    ...
    parser.add_argument("--grid", type=str, choices=["square", "raw_depth"], default="square")
    parser.add_argument("--mask_size", type=int, default=128, help="Square-mask size; ignored for raw_depth")
...
def main():
    args = parse_args()
    if args.grid == "square" and args.mask_size <= 0:
        raise ValueError("--mask_size must be positive for square masks")
    ...
    for index, rgb_name in enumerate(pbar):
        ...
        target_depth = load_target_depth(depth_path, args.grid, args.mask_size)
    ...
    metadata = {
        ...
        "grid": args.grid,
        "size": args.mask_size if args.grid == "square" else None,
